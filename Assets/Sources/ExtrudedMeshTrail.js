...
function LateUpdate () {
	...
	&& 
	now > 
	(sections[sections.length - 1] as ExtrudedTrailSection).time 
	+ time) {
		sections.Pop();
	}
	...
	for (var i=0;i<sections.length;i++)
	{
		if (autoCalculateOrientation)
		{
			if (i == 0)
			{
				var direction = (sections[0] as ExtrudedTrailSection).point - (sections[1] as ExtrudedTrailSection).point;
			...
			{	
				direction = (sections[i] as ExtrudedTrailSection).point - (sections[i+1] as ExtrudedTrailSection).point;
				...
				previousRotation = rotation;
				finalSections[i] = worldToLocal * Matrix4x4.TRS((sections[i] as ExtrudedTrailSection).point, rotation, Vector3.one);
			}
		...
		else
		{
			...
			{
				finalSections[i] = worldToLocal * (sections[i] as ExtrudedTrailSection).matrix;
			}
		}
	}
