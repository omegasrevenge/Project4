...
function LateUpdate () {
	...
	// Remove old sections
	while (sections.length > 0 && now > (sections[sections.length - 1] as TronTrailSection).time + time) {
		sections.Pop();
	}

	// Add a new trail section
	if (sections.length == 0 || ((sections[0] as TronTrailSection).point - position).sqrMagnitude > minDistance * minDistance)
	{
	...
	var previousSection : TronTrailSection = sections[0] as TronTrailSection;
	var currentSection : TronTrailSection = sections[0] as TronTrailSection;
	...
	for (var i=0;i<sections.length;i++)
	{
		...
		currentSection = sections[i] as TronTrailSection;
