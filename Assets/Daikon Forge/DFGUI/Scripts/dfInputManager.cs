...
public class dfInputManager : MonoBehaviour
{
	...
	public void OnEnable()
	{
		...
		if( this.adapter == null )
		{
			...
			var inputAdapter =
				...
				.Where( c => typeof( IInputAdapter ).IsAssignableFrom( c.GetType() ) )
				.FirstOrDefault();
	...
	private void processKeyEvent( EventType eventType, KeyCode keyCode, EventModifiers modifiers )
	{
		...
		var controlKey = ( modifiers & EventModifiers.Control ) == EventModifiers.Control;
			...
			public bool Process( TouchRaycast info )
			{
				...
				if( info.Phase == TouchPhase.Canceled || info.Phase == TouchPhase.Ended )
				{
					...
					if( touches.Count == 1 )
					{
						...
						var singleEventArgs = (dfTouchEventArgs)touches.Values.First();

						control.OnMouseEnter( singleEventArgs );
						control.OnMouseDown( singleEventArgs );
	...
	private class MouseInputManager
	{
		...
		/// <summary> The length of time between OnMouseHover notifications </summary>
		private const float HOVER_NOTIFICATION_FREQUENCY = 0.1f;

		/// <summary> The length of time the mouse must hover before OnMouseHover notifications begin </summary>
		...
		public void ProcessInput( IInputAdapter adapter, Ray ray, dfControl control, bool retainFocusSetting )
		{
			...
			if( dragState == dfDragDropState.Dragging )
			{
				...
				if( buttonsReleased == dfMouseButtons.None )
				{
					...
					if( control != null )
					{
						...
						if( Vector2.Distance( position, lastPosition ) > 1f )
						{
							var dragArgs = new dfDragEventArgs( control, dragState, dragData, ray, position );
				...
				if( control != null && control != activeControl )
				{
					...
					// change the state from Dragging (which is not a valid state for
					// a drop event) then just cancel the operation
					if( !dragArgs.Used || dragArgs.State == dfDragDropState.Dragging ) dragArgs.State = dfDragDropState.Cancelled;
			...
			if( buttonsReleased != dfMouseButtons.None )
			{
				...
				if( activeControl == null )
				{
					setActive( control, position, ray );
					return;
				}

				// If the left button is released over the same control it was pressed on,
			...
			if( activeControl != null && activeControl == control )
			{
				...
				if( mouseMoveDelta.magnitude == 0 && Time.realtimeSinceStartup - lastHoverTime > HOVER_NOTIFICATION_FREQUENCY )
				{
